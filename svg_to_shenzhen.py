...
class Svg2ModExportPretty( Svg2ModExport ):
		...
		def _write_wirepad( self ):
			...
			bb_width_center = viewbox_w/2
			bb_height_center = viewbox_h/2	
			bb_scaling = viewbox_h/height
			...
			for item in items:
				...
				if (item.name == "Drill"): 
					...
					for drill in item.items:
						count = count + 1;
						print vars(drill)
						pad_x = (drill.center.x-bb_width_center)/bb_scaling
						pad_y = (drill.center.y-bb_height_center)/bb_scaling
